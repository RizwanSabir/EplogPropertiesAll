  ...
  return (
    <Fragment>
      <Row className="mb-3 d-flex align-items-center">
        ...
        <Col md="3">
          <div className="mb-3">
            <Label className="form-label" htmlFor="validationCustom01">Category</Label>
            <Dropdown setGlobalFilter={setGlobalFilter}/>
            {/* <select
...
const Dropdown = ({ setGlobalFilter }) => {
  ...
  const handleChange = (event) => {
    ...
    if (value === "Show All") {
      setSelectedOptions(["Technology", "Photography", "Food", "Lifestyle"]);
      setGlobalFilter(""); // Reset global filter to show all
    } else if (value) {
      setSelectedOptions((prev) =>
        prev.includes(value)
          ? prev.filter((item) => item !== value) // Deselect the item
          : [...prev, value] // Select the item
      );
      setGlobalFilter(value); // Set the global filter
    } else {
      // Reset selection when empty
      setSelectedOptions([]);
      setGlobalFilter("");
    }
  ...
  const options = [
    ...
    "Food",
    "Lifestyle",
    ...(selectedOptions.length > 0 ? ["Show All"] : []),
  ];
